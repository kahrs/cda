...
int nonames = 0, nocomments = 0, nofamilies = 0, PCB = 0, netlist = 0;
...
void
outpcbsigs(Signal *ss)
{
	Netpoint *np;
	Chip *cc;
	fprint(1, "%s", ss->name);
	for(np = (Netpoint *) ss->coords; np; np = np->next) {
		if (cc = (Chip *)symlook(np->chip, S_CHIP, (void *)0))
			fprint(1, " %s-%d", cc->typename, np->pinno);
		else	fprint(1, " %s-%d", np->chip, np->pinno);
	}
	fprint(1, "\n");
}
...
out(void) 
{
		if (!netlist) symtraverse(S_TYPE, outtypes);
		if (!netlist) symtraverse(S_CHIP, outchips);
		if (PCB) symtraverse(S_SIGNAL, outpcbsigs);
		else symtraverse(S_SIGNAL, outsigs);
}
