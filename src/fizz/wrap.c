...
#ifdef PLAN9
			if((cbfd = create(cbfile, O_RDWR, 0666)) < 0){
#endif
#ifndef PLAN9
			if((cbfd = creat(cbfile, O_RDWR)) < 0){
#endif
			perror(cbfile);
			exits("create");
		}
...
#endif
#ifndef PLAN9
	if((xyfd = creat(xyfile, O_WRONLY)) < 0){
		perror(xyfile);
		exits("creat");
	}
	if(hnflag && ((hnfd = creat(hnfile, O_WRONLY)) < 0)){
		perror(hnfile);
		exits("creat");
	}
	if(brflag) 
		if((kpfd = creat(kpfile, O_WRONLY)) < 0){
			perror(kpfile);
			exits("creat");
		}
#endif
