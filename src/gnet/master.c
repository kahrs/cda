...
void Master::process()
{
	extern int kflag,mflag;
	extern int strlen(char *);
	extern char *stem;
	findmbb();
	labels->walk(macros,(int (Vector::*)(Text *))&BoxList::pins);
	labels->walk(boxes,(int (Vector::*)(Text *))&BoxList::parts);
	labels->walk(boxes,(int (Vector::*)(Text *))&BoxList::pins);
	labels->walk(wires,(int (Vector::*)(Text *))&WireList::nets);
	if (kflag)
		labels->walk(boxes,(int (Vector::*)(Text *))&BoxList::kahrs);
	wires->prop();
	wires->prop1();
	macros->nets(wires);
	boxes->nets(wires);
	labels->walk(labels,(int (Vector::*)(Text *))&TextList::prop);	/* graw->jraw hack */
	labels->walk(macros,(int (Vector::*)(Text *))&BoxList::parts);
	boxes->apply((int*)wires,(void (Thing::*)(int *))&Box::getname);
	macros->absorb(boxes);
	macros->put(stdout);
