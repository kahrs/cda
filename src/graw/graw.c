...
{
	screen.r = inset(r,3);
	stage = &screen; /* a wing and a prayer */
	show();
}
...
#endif
#ifndef PLAN9
        xtbinit(0, "Graw", &argc, argv, fallbacks);
        r = inset(screen.r, 4);
...
dobut3(void)
{
	...
	case EDIT:
		buf[0] = 0;
		if (*mygetline("edit file: ",buf) && (this = editfile(buf)))
			show();
		break;
	case READ:
		buf[0] = 0;
		if (*mygetline("read file: ",buf) && readfile(buf)) {
	...
	case WRITE:
		...
		if (*mygetline("write file: ",buf) && (fp = fopen(buf,"w")) != 0) {
		...
		break;
	case EXIT:
		if (*mygetline("really quit? ",buf) == 'y')
			exits(0);
		break;
	case NEW:
		this = newfile(intern(""));
