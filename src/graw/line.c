...
void
prect(FILE *fp, char *s, Rectangle r)
{
...
putline(FILE *fp,Line *l,int nonm)
{
	...
	for (; l; l = l->next)
		...
		case LINE:
			prect(fp,"l ", rshift(raddp(l->r,dp),i));
			break;
		case BOX:
			prect(fp,"b ", rshift(raddp(l->r,dp),i));
			break;
		case DOTS:
			prect(fp,"d ",rshift(raddp(l->r,dp),i));
			break;
		case MACRO:
			prect(fp,"z ",rshift(raddp(l->r,dp),i));
			break;
		case STRING:
			s = (String *)l;
			sp = rshift(raddp(s->r,dp),i).min;
			fprintf(fp,"s \"%s\" %d %d %d\n",s->s,s->code,sp.x,sp.y);
			break;
		case INST:
			sp = rshift(raddp(l->r,dp),i).min;
			fprintf(fp,"i \"%s\" %d %d\n",((Inst *)l)->name,sp.x,sp.y);
			break;
		}
		else switch(l->type) {
		case REF:
